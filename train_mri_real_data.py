...
from models.madun import MADUN
from C2R import *
...
parser.add_argument('--layer_num', type=int, default=10, help='phase number of ISTA-Net-plus')
...
parser.add_argument('--ratio', type=float, default=0.20, help='sampling ratio for the MRI mask')
...
parser.add_argument('--model_dir', type=str, default='model_cnn_small', help='trained or pre-trained model directory')
...
### whether I need to SGD as the optimizers?

### define some data from some args
...
## specify the save_interval
save_interval = 20

### import the mask
### specify the mask, use the mat file
#Phi_data_Name = './masks/mask2r0.33.mat'
#Phi_data_Name = f'/home/ytzhao/CV_Project/ISTA-Net-PyTorch/masks/mask{args.mask_type}{args.ratio}.mat'
Phi_data_Name = f'/home/huanzheng/video/DSO_delivery/CV_Project/ISTA-Net/masks/mask{args.mask_type}{args.ratio:.2f}.mat'
print(Phi_data_Name)
...
Noise_name = f'/home/huanzheng/video/DSO_delivery/CV_Project/ISTA-Net/mri_noise/noise{args.mask_type}{args.ratio:.2f}.mat'
print(Noise_name)
...
##### loading the BSD1500 data
class RealData(Dataset):
    def __init__(self, mode='train', root_dir='/home/huanzheng/video/DSO_delivery/CV_Project/ISTA-Net/masks/bsds2r0.20.mat', tag=1400):

        x = sio.loadmat(root_dir)['Data']
        ...
        if args:
            Noise_name = f'/home/huanzheng/video/DSO_delivery/CV_Project/ISTA-Net/masks/noise{args.mask_type}{args.ratio:.2f}_new.mat'
            print(Noise_name)
            N = sio.loadmat('%s' % ( Noise_name))
            self.Noise = torch.from_numpy(N['N'])
...
data_loader = torch.utils.data.DataLoader(dataset=RealData(mode='train'),
                                   batch_size=args.batch_size, shuffle=True)

val_loader = torch.utils.data.DataLoader(dataset=MRIData(mode='test'),
                                   batch_size=1, shuffle=False)
train_dataset = RealData(mode='train')
...
model_dir = "./%s/Real1500_Sup_layer%d_mask%s_ratio%.4f_lr%.4f_optim%s_fixnoise"% (args.model_dir, args.layer_num, args.mask_type, args.ratio, args.lr, args.optimizer)
log_file_name = "./%s/Log_Real1500_Sup_layer%d_mask%s_ratio%.4f_lr%.4f_optim%s_fixnoise.txt" % (args.log_dir,  args.layer_num, args.mask_type, args.ratio, args.lr, args.optimizer)
...
#        batch_n = batch_n.type(torch.FloatTensor).to(device)
...
#        print(batch_n)
    ...
    if epoch_i % 1000 == 0:
        ...
        eval_status = evaluate(model, val_loader, mask, device)
        psnr_list.append(eval_status['PSNR'])
        ssim_list.append(eval_status['SSIM'])
